...
function convertToOrigin<T>(str: string): null | T {
  try {
    ...
    if (Number.isNaN(data.expire))
      return null // 之前IOS的缓存可能会存储NaN
    if (data.expire && safeDate().getTime() > data.expire)
      return null
    return data.origin
  }
  catch (e) {
...
function convertToStr<T = string>(origin: T, expire?: Date | null) {
...
export default (type: 'local' | 'session' | 's' | 'l' = 'local') => {
  ...
  return {
    has(originKey: string): boolean {
      const key = convertKey(originKey)
      return !!storage.getItem(key)
    },
    get <T = string>(originKey: string): null | T {
      const key = convertKey(originKey)
    ...
    set <T = string>(originKey: string, data: T, expire = null): void {
      const key = convertKey(originKey)
      const str = convertToStr(data, expire)
      return storage.setItem(key, str)
    },
    remove(originKey: string): void {
      const key = convertKey(originKey)
